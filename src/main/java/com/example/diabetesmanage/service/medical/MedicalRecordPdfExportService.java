...
public class MedicalRecordPdfExportService {
    ...
    private void addInternalMedicine(Document document, MedicalRecordDetailView view,
        ...
        InternalMedicineSection section = view.getInternalMedicine();
        ...
        for (MedicationChip med : section.getMedications()) {
            document.add(new Paragraph("- " + safe(med.getName()) + ": " + safe(med.getDose()), normalFont));
        }
    ...
    private void addBloodCount(Document document, MedicalRecordDetailView view,
        ...
        for (MedicalFieldItem item : view.getBloodCount().getItems()) {
    ...
    private void addBiochemistry(Document document, MedicalRecordDetailView view,
        ...
        BiochemistrySection section = view.getBiochemistry();
    ...
    private void addFieldTable(Document document, List<MedicalFieldItem> items, Font font)
        ...
        for (MedicalFieldItem item : items) {
            if (item == null) {
                continue;
            }
            addRow(table, safe(item.getLabel()) + ":", item.getDisplayValue(), font);
        }

        document.add(table);
    }

    private void addLabRow(PdfPTable table, MedicalFieldItem item, Font font) {
        PdfPCell labelCell = new PdfPCell(new Phrase(safe(item.getLabel()), font));
        PdfPCell valueCell = new PdfPCell(new Phrase(item.getDisplayValue(), font));
        PdfPCell rangeCell = new PdfPCell(new Phrase(safe(item.getReferenceRange()), font));
